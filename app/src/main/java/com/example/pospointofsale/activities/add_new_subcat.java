...
public class add_new_subcat extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        subcatagoryname = findViewById(R.id.ed_username);
            ...
            Task<Uri> urlTask = uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if (task.isSuccessful()) {
                        ...
                        if (downloadUri != null) {
                            String subcatname = subcatagoryname.getText().toString().trim();
                            String photoStringLink = downloadUri.toString(); //YOU WILL GET THE DOWNLOAD URL HERE !!!!
                            c1.setCatimg(photoStringLink);
                            c1.setCatname(subcatname);
                            c1.setId("");
                            c1.setCount("");
                            ...
                            DocumentReference dr = fd.collection(a[0]).document(a[1]).collection(a[2]).document(a[3]).collection("subcatagory").document();
                            dr.set(c1).addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onSuccess(Void v) {
                                    ...
                                    String path = getIntent().getStringExtra("refrence").trim();
                                    String username = getIntent().getStringExtra("username").trim();
                                    Intent i = new Intent(add_new_subcat.this, sub_cat.class);
                                    i.putExtra("refrence",path);
                                    i.putExtra("username",username);
                                    startActivity(i);
                                    Toast.makeText(add_new_subcat.this, "success", Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
